...
Page({
   ...
   */
	onLoad: function (options) {
		...
		app.new_login(aRes => {
			template.GetReductionCard(that, options.couponsId, options.orderId, 2)
		})
		setTimeout(function () {
			if (that.data.coupon != null) {
				setInterval(function () {
					...
					that.data.coupon.endtime = template.formatDuring(endtime)
	...
	getreducecard: function () {
		var that = this
		app.new_login(function () {
			template.GetReductionCard(that, that.data.couponsId, that.data.dbOrder, 1)
		})
	},
  /**
   * 生命周期函数--监听页面初次渲染完成
