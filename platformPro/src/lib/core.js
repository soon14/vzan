...
/**********************************封装工具类**********************************************************/
var tools = {
  async checkVaildCard(vm) {
    var isShowIllegal = false
    var cacheShowIllegal = tools.cacheGlobalData('isShowIllegal')

    if (typeof cacheShowIllegal === 'boolean') {
      vm.isShowIllegal = cacheShowIllegal
      vm.$apply()
      return
    }

    var vaildCardInfo = await core.checkCardState()

    if (!vaildCardInfo.isok) {
      if (vaildCardInfo.Msg.match('违反平台规则')) {
        isShowIllegal = true
      } else {
        tools.showModalCancle(vaildCardInfo.Msg)
      }
    }

    tools.cacheGlobalData('isShowIllegal', isShowIllegal)

    vm.isShowIllegal = isShowIllegal
    vm.$apply()
  },
  repalceHttps(url) {
  ...
  async checkRegister() {
    ...
    var myCardId = 0
    var cacheResult = tools.cacheGlobalData('check-register')
    ...
    tools.cacheGlobalData('check-register', result)
    return result
  },
  getLocation() {
    return new Promise(resolve => {
      wx.getLocation({
        success: (res) => {
          console.log(res, '获取位置授权成功')
          resolve(res)
        },
        complete: (res) => {
          console.log(res, '获取位置授权完成')
          resolve(res)
        }
      })
    })
  ...
  getSystemInfo: function () {
    var systemInfo = tools.cacheGlobalData('systemInfo')
    if (systemInfo) {
      return systemInfo
    }
    return new Promise((resolve) => {
      wx.getSystemInfo({
        success: (res) => {
          tools.cacheGlobalData('systemInfo', res)
          resolve(res)
        },
        fail: () => {
          resolve(false)
          tools.freeToast('获取手机系统信息失败', 'none')
        }
      })
    })
  },
...
var core = {
  ...
  async GetConfig(location) {
    ...
    return http.get(addr.GetConfig, {
      appId: app.globalData.appid,
  ...
  //平台店铺类别
  async GetStoreCategoryLevel() {
    let app = wepy.$instance;
    let user = await core.getUserInfo()
    return new Promise(function (resolve, reject) {
      http.get(addr.GetStoreCategoryLevel, {
        appId: app.globalData.appid,
        sessionKey: user.loginSessionKey,
      }).then(data => {
        if (data.isok) {
  ...
  //平台店铺对应类别的分类
  async GetStoreCategory(_param) {
    let appid = wepy.$instance.globalData.appid;
    let user = await core.getUserInfo()
    let defaultParam = {
      appId: appid,
      sessionKey: user.loginSessionKey,
      isFirstType: 2, //level1:2大类,level2:1小类,其他:选单独大类时填写3
      parentId: "", //单独大类的id
    }
    let param = tools.extend(defaultParam, _param);
    //debugger
    return http.get(addr.GetStoreCategory, param);
  },
  //获取店铺列表
  async GetStoreList(_param) {
    let appid = wepy.$instance.globalData.appid;
    let user = await core.getUserInfo()
    let defaultParam = {
      appId: appid,
    ...
    let param = tools.extend(defaultParam, _param);
    return http.get(addr.GetStoreList, param);
  },

  /**************************************************分类信息*****************************************************/
  //获取帖子列表
  async GetMsgList(param) {
    let app = wepy.$instance;
    let user = await core.getUserInfo()
    return http.get(addr.GetMsgList, {
      appId: app.globalData.appid,
      userId: user.Id,
      sessionKey: user.loginSessionKey,
      msgTypeId: param.msgTypeId || "",
  ...
  async countUpData(id, actionType, dataType) {
    let app = wepy.$instance;
    let user = await core.getUserInfo()
    let aid = await core.getAid()
  ...
  //获取点赞列表
  async GetOtherFavoriteList(id, actiontype, type, datatype) {
    let user = await core.getUserInfo()
    let aid = await core.getAid()
    let appid = wepy.$instance.globalData.appid
    return http.post(addr.GetOtherFavoriteList, {
      userId: user.Id,
      sessionKey: user.loginSessionKey,
      actiontype: actiontype, //0收藏 1点赞 2关注，3看过，4私信
      mycardid: id,
      aid: aid,
      datatype: datatype || 0,
      pageindex: 1,
  ...
  //获取评论列表
  async GetMsgComment(id, type) {
    let app = wepy.$instance;
    let user = await core.getUserInfo()
    return http.get(addr.GetMsgComment, {
      appId: app.globalData.appid,
      sessionKey: user.loginSessionKey,
      Id: id,
      actionType: type, //0帖子，1商品
    })
  },
  //发送评论
  async AddComment(id, type, value) {
    let app = wepy.$instance;
    let user = await core.getUserInfo()
    return http.post(addr.AddComment, {
      appId: app.globalData.appid,
      sessionKey: user.loginSessionKey,
      userId: user.Id,
      Id: id,
      dataType: type, //0帖子，1商品
      CommentDetail: value
    })
  },
  //重组数据 
  ...
  async GetStoreGoods(_param) {
    ...
    let param = tools.extend(defaultParam, _param);
    return http.get(addr.GetStoreGoods, param);
  },
  //获取产品详情
  ...
  //获取独立小程序二维码
  async GetStoreCodeImg(storeid) {
    let user = await this.getUserInfo();
    let defaultParam = {
      sessionKey: user.loginSessionKey,
      storeid
    };
    return http.post(addr.GetStoreCodeImg, defaultParam);
  },
  //绘制独立小程序二维码
  async drawStoreCode(name, codeUrl, canvasSize) {
    //功能函数 ，限制长度
    var limitLength = function (v) {
      if (v.length > 8) {
        v = v.slice(0, 6) + '...'
      }
      return v
    ...
    var bgUrl = '/img/canvas-bg/store_bg2.png'
    var imgRect = await core.selectNode('#canvasShop')
    // var suffix = '?x-oss-process=image/resize,l_100,image/circle,r_100/format,png'
    // var qrcode = codeUrl
    var qrcodeDown = await tools.downFile(codeUrl)
    codeUrl = qrcodeDown.tempFilePath
    const ctx = wx.createCanvasContext('canvas-shop')

    //底图
    // try {
    //   //core.selectNode 可能返回null
    //   ctx.drawImage(bgUrl, 0, 0, imgRect[0].width, imgRect[0].height)
    // } catch (error) {
    //   // ctx.drawImage(bgUrl, 0, 0, 300, 450)
    // }
    ctx.drawImage(bgUrl, 0, 0, canvasSize.width, canvasSize.height)

    //二维码
    var codeX = (52 / 600) * canvasSize.width;
    var codeY = (637 / 900) * canvasSize.height;
    var textX = (420 / 600) * canvasSize.width;
    var textOneY = (750 / 900) * canvasSize.height;
    console.log(codeX, codeY);
    // ctx.drawImage(codeUrl, 26, 330, 88, 88)
    ctx.drawImage(codeUrl, codeX, codeY, 112.5, 112.5)

    //店铺名
    ctx.setFillStyle('#333')
    ctx.setFontSize(14)
    ctx.setTextAlign('center')
    ctx.fillText("点击图中小程序码", textX, textOneY)
    ctx.fillText("长按识别即可购买", textX, textOneY + 20)
  ...
  //获取运费
  async getFreightFee(province, city, goodcartids, storeid) {
    var user = await core.getUserInfo()
  ...
  async cancleOrder(orderid) {
    ...
    if (result.confirm) {
      ...
      tools.handleResult(data, async () => {
        ...
        tools.goRedirecto('/pages/shoppingCarts/my-order/index')
      }, () => {
        wx.hideLoading()
  ...
  async confirmOrder(orderid) {
    ...
    if (result.confirm) {
      ...
      tools.handleResult(data, async () => {
        ...
        tools.goNewPage('/pages/shoppingCarts/my-order/goodOlt?orderid=' + orderid)
      }, () => {
        wx.hideLoading()
        tools.freeToast('收货失败', 'none', 500)
      })
    }
  },
  /************************************************私信************************************************** */
  connectSocket: async function () {
    ...
    wx.connectSocket({
      url: 'wss://dzwss.xiaochengxu.com.cn?appId=' + appid + '&userId=' + fuserid,
      header: {
        'content-type': 'application/json'
      },
      method: "GET"
    ...
    wx.onSocketMessage(function (res) {
      ...
      if (currentPage.route == "pages/connected/private-letter/index") {
      ...
      } else if (currentPage.route == "pages/join/join-index/index") {
        //我的页面有新消息时，显示红点
        currentPage.setData({
         'hasNewMsg': true
        });
        ...
        //将未读数据缓存到全局变量里面
        tools.freeToast('您有一条新私信', 'none', 500)
  ...
  markUnreadMsg: function (msg) {
   ...
   var model = {
     lastNews: msg.msg,
     lastSendDate: msg.sendDate
   }
   var unReadMsgInfo =  tools.cacheGlobalData('unReadMsgInfo')
   unReadMsgInfo[key] = model
   tools.cacheGlobalData('hasNewMsg', true)
  },
  /**
   * 获得联系列表
   * @param {Number} pageindex 页码
  ...
  /**
   * 增加动态
   * @param actionType 0：收藏 ，1：点赞， 2：关注 ， 3：看过， 4：私信 5，评论 ，6 转发
   * @param callback 成功之后的回调
   * @param targetPage 当前页实例
   * @param otherCardId  另外的卡id
   * @param dataType  帖子= 0,  商品 = 1, 评论 = 2, 名片=3(默认),
   */
  addDynamic: async function (actionType, otherCardId, callback, targetPage, dataType) {
    var vm = targetPage;
    dataType = typeof dataType !== 'undefined' ? dataType : 3 //(默认)
    var userInfo = await this.getUserInfo()
    var aid = await this.getAid()
    http
      .post(addr.AddFavorite, {
      ...
      .then(res => {
        //假如有回调触发回调
        typeof callback === "function" && callback.call(vm, res);
        vm.$apply();
      });
  },
  //获取人脉圈数据
  getConnectionsList: async function (options_) {
    var user = await core.getUserInfo()
    ...
    var options = tools.extend(defaultSetting, options_)
    return http.post(addr.GetConnectionsList, options)
  },
  //获取行业数据
  getIndustry: async function () {
    var user = await core.getUserInfo()
    var sessionKey = user.loginSessionKey
    return http.post(addr.GetIndustryList, {
  ...
  getAreaList: async function () {
    var user = await core.getUserInfo()
    var sessionKey = user.loginSessionKey
    return http.post(addr.GetAreaList, {
      sessionKey: sessionKey
    });
  },
  //获取我的名片信息
  getMyCard: async function () {
    ...
    return http.post(addr.GetMyCard, {
      appid,
      myuserid: userInfo.Id, //我的userid
      userid: userInfo.Id, //名片的userid
      aid: aid,
      sessionKey: userInfo.loginSessionKey
    })
  },
  //入驻商铺
  addStore: function (options_) {
    return http.post(addr.AddStore, options_)
  },
  //获得店铺详情
  getStoreDetail: async function (_param) {
    var appId = wepy.$instance.globalData.appid;
    var userInfo = await core.getUserInfo();
    var defaultParam = {
      appId,
    ...
    var param = tools.extend(defaultParam, _param)
    return http.get(addr.GetStoreDetail, param);
  },
  //获得活动轨迹
  getActivityLog: async function (userid, pageIndex) {
    console.log(userid, '用户id')
    var user = await core.getUserInfo()
    userid = userid || user.Id
    return http.post(addr.GetActivityLog, {
      sessionkey: user.loginSessionKey,
      userid: userid,
      pageindex: pageIndex || 1
  ...
  //获取我的动态
  getMyActivity: async function (myCardId, pageIndex) {
    var appid = wepy.$instance.globalData.appid
    return http.get(addr.GetMsgByUserId, {
      appid: appid,
      mycardId: myCardId,
      pageindex: pageIndex || 1
    })
  ...
  //删除帖子
  delMyFavoriteOrMyMsg(option) {
    var defult = {
      appId: wepy.$instance.globalData.appid,
      Id: 0, //帖子ID
      myCardId: 0, //我的名片Id
      delType: 0 //帖子类型 0:我发布的 1:我收藏的
    }
    return http.post(addr.DelMyFavoriteOrMyMsg, tools.extend(defult, option))
  },
  //获取收藏的动态
  async getListMyFavoriteMsg(pageIndex) {
    var user = await core.getUserInfo()
    var defult = {
      appId: wepy.$instance.globalData.appid,
      userid: user.Id || 0, //我的名片Id
      pageIndex: pageIndex || 1
    }
    return http.post(addr.GetListMyFavoriteMsg, defult)
  },
  //获取交互列表
  async getOtherFavoriteList(option) {
    let user = await core.getUserInfo()
    let aid = await core.getAid()
    let appid = wepy.$instance.globalData.appid
    ...
    let default_ = {
      ...
      aid: aid,
      appid,
      actiontype: 3, //0收藏 1点赞 2关注，3看过，4私信 5.评论
      mycardid: 0, //变量
      datatype: 3,
      pageindex: 1,
      pagesize: 8,
    ...
    return http.post(addr.GetOtherFavoriteList, tools.extend(default_, option))
  },
  //获取我的评论列表 version2
  async getMsgComment(option) {
    var default_ = {
      appId: wepy.$instance.globalData.appid,
      Id: 0, //帖子ID 或 商品ID
      actionType: 0, // 0:帖子 1:商品
      keyMsg: '', //搜索关键字
      userId: 0, //与Id互斥 大于0为用户的评论
      pageIndex: 1,
      pageSize: 10
    }

    return http.post(addr.GetMsgComment, tools.extend(default_, option))
  },
  //删除评论
  delMsgComment(id){
    let appid = wepy.$instance.globalData.appid;
    return http.post(addr.DeleteMsgComment,{
      appid,
      Id:id,
    })
  },
  //获取数据雷达 type: -1：统计 0：本月 1：上个月
  async getRadarData(type) {
    let user = await core.getUserInfo()
    let aid = await core.getAid()
    return http.post(addr.GetRadarData, {
      userid: user.Id,
      aid,
      sessionkey: user.loginSessionKey,
      type
  ...
  //获取名片码呦
  async getMyCardCodeUrl(url, userid) {
    let user = await core.getUserInfo()
    let aid = await core.getAid()
    return http.post(addr.GetMyCardCodeUrl, {
      userid: userid,
      aid,
      sessionkey: user.loginSessionKey,
      pageurl: url
  ...
  //画名片卡
  async drawCanvasCard(info, encryptPhone, url, canvasWidth, canvasHeight, canvasId, ratio) {
    canvasId = canvasId || 'canvas-card'
    ratio = ratio || 1

    //功能函数 ，限制长度
    var limitLength = function (v, limit, remain) {
      if (v.length > limit) {
        v = v.slice(0, remain) + '...'
      }
      return v
    ...
    //绘制准备
    var imgPath = '/img/canvas-bg/card_bg.png'
    var suffix = '?x-oss-process=image/resize,l_100,image/circle,r_100/format,png'
    var arcAvator = tools.repalceHttps(info.ImgUrl + suffix)
    var qrcode = tools.repalceHttps(url + suffix)


    //换取可用路径
    var srcFromDown = await tools.downFile(arcAvator)
    var qrcodeDown = await tools.downFile(qrcode)
    arcAvator = srcFromDown.tempFilePath
    qrcode = qrcodeDown.tempFilePath

    //限制姓名 公司名、地址名长度
    info.CompanyName = limitLength(info.CompanyName, 17, 15)
    info.Address = limitLength(info.Address, 17, 15)
    info.Name = limitLength(info.Name, 13, 11)
    const ctx = wx.createCanvasContext(canvasId)

    //底图
    ctx.drawImage(imgPath, 0, 0, canvasWidth, canvasHeight)
    ...
    var NameX = tools.getRelativeValue(250 * ratio, 29.5 * ratio, canvasWidth)
    var NameY = tools.getRelativeValue(303.5 * ratio, 48 * ratio, canvasHeight)
    ctx.fillText(info.Name, NameX, NameY)
    ctx.setFillStyle('#666666')
    ctx.setFontSize(12 * ratio)
    var DepartmentX = tools.getRelativeValue(250 * ratio, 29.5 * ratio, canvasWidth)
    var DepartmentY = tools.getRelativeValue(303.5 * ratio, 67 * ratio, canvasHeight)
    ctx.fillText(info.Department + '  ' + info.Job, DepartmentX, DepartmentY)
    var encryptPhoneX = tools.getRelativeValue(250 * ratio, 29.5 * ratio, canvasWidth)
    var encryptPhoneY = tools.getRelativeValue(303.5 * ratio, 85 * ratio, canvasHeight)
    ctx.fillText(encryptPhone, encryptPhoneX, encryptPhoneY)
    ctx.setFillStyle('#333')
    var CompanyNameX = tools.getRelativeValue(250 * ratio, 29.5 * ratio, canvasWidth)
    var CompanyNameY = tools.getRelativeValue(303.5 * ratio, 120 * ratio, canvasHeight)
    ctx.fillText(info.CompanyName, CompanyNameX, CompanyNameY)
    ctx.setFillStyle('#666')
    var AddressX = tools.getRelativeValue(250 * ratio, 29.5 * ratio, canvasWidth)
    var AddressY = tools.getRelativeValue(303.5 * ratio, 138 * ratio, canvasHeight)
    ctx.fillText(info.Address, AddressX, AddressY)
    //头像
    var arcAvatorXS = tools.getRelativeValue(250 * ratio, 189 * ratio, canvasWidth)
    var arcAvatorYS = tools.getRelativeValue(303.5 * ratio, 42 * ratio, canvasHeight)
    var arcAvatorXE = tools.getRelativeValue(250 * ratio, 220 * ratio, canvasWidth)
    var arcAvatorYE = tools.getRelativeValue(303.5 * ratio, 73 * ratio, canvasHeight)
    ctx.drawImage(arcAvator, arcAvatorXS, arcAvatorYS, arcAvatorXE - arcAvatorXS, arcAvatorYE - arcAvatorYS)
    //二维码
    var qrcodeXS = tools.getRelativeValue(250 * ratio, 146 * ratio, canvasWidth)
    var qrcodeYS = tools.getRelativeValue(303.5 * ratio, 200 * ratio, canvasHeight)
    var qrcodeXE = tools.getRelativeValue(250 * ratio, 235 * ratio, canvasWidth)
    var qrcodeYE = tools.getRelativeValue(303.5 * ratio, 289 * ratio, canvasHeight)
    ctx.drawImage(qrcode, qrcodeXS, qrcodeYS, qrcodeXE - qrcodeXS, qrcodeYE - qrcodeYS)
    ctx.stroke()
    ctx.draw()
  },
  async applyStoreApp() {
    var user = await core.getUserInfo()
    return http.post(addr.ApplyStoreApp, {
      userid: user.Id,
      sessionkey: user.loginSessionKey
    });
  },
  async drawCanvasShop(name, qrcode, canvasWidth, canvasHeight, canvasId, ratio) {
    canvasId = canvasId || 'canvas-shop'
    ratio = ratio || 1
    ...
    var bgUrl = '/img/canvas-bg/shop_bg.png'
    qrcode = tools.repalceHttps(qrcode)
    var qrcodeDown = await tools.downFile(qrcode)
    qrcode = qrcodeDown.tempFilePath
    const ctx = wx.createCanvasContext(canvasId)
    ...
    var qrXS = tools.getRelativeValue(250 * ratio, 16.5 * ratio, canvasWidth)
    var qrYS = tools.getRelativeValue(303.5 * ratio, 96 * ratio, canvasHeight)
    var qrXE = tools.getRelativeValue(250 * ratio, 106.5 * ratio, canvasWidth)
    var qrYE = tools.getRelativeValue(303.5 * ratio, 186 * ratio, canvasHeight)

    //二维码
    ctx.drawImage(qrcode, qrXS, qrYS, qrXE - qrXS, qrYE - qrYS)

    //店铺名
    ctx.setFillStyle('#fff')
    ctx.setFontSize(12 * ratio)
    ctx.setTextAlign('center')
    var shopNameCaneterX = tools.getRelativeValue(250 * ratio, (250 / 2) * ratio, canvasWidth)
    var shopNameY = tools.getRelativeValue(303.5 * ratio, 262 * ratio, canvasHeight)
    ctx.fillText('店铺名：' + limitLength(name), shopNameCaneterX, shopNameY)
    //render
    ctx.stroke()
    ctx.draw()
  },
   ...
   */
  async getCoupon(couponId, storeid) {
    var user = await core.getUserInfo()
   ...
   * @param {int} storeId 
   */
  async getStoreCodeImg(storeId, path) {
    var user = await core.getUserInfo()
    return http.post(addr.GetStoreCodeImg, {
      sessionkey: user.loginSessionKey,
      storeid: storeId,
      pageurl: path || ''
    })
  ...
  //检查名片是否被禁用
  async checkCardState() {
    var user = await core.getUserInfo()
    return http.post(addr.CheckCardState, {
      sessionkey: user.loginSessionKey,
      userid: user.Id
    })
  }
