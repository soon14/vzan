...
Page({
  ...
  setCondition: function () {
    ...
    if (isFirstClick) {
      for (var i = 0; i < item5.length; i++) {
        item5[i].price = (Number(item5[i].nums) * Number(this.data.item5_1[i].price))
      ...
      if (item5.length != 0 && isFirstClick == false) {
        for (var i = 0; i < item5.length; i++) {
          if (typeof (this.data.item5_1[i].price) == "string") {
            item5[i].price = (Number(this.data.item5_1[i].price) * Number(item5[i].nums))
          }
        }
        this.setData({
          item5_1: item5
  ...
  setCondition1: function () {
    ...
    // var nums = 
    // goodslist[index].good.carCount = this.data.goodslist_2[index].nums
    // goodslist[index].good.carCount = (goodslist[index].good.carCount) - this.data.nums
    // nums = 0
  ...
  showallNotice: function () {
    var isallNotice = this.data.isallNotice
    this.setData({ isallNotice: !this.data.isallNotice })
  },
  // 改变changeChoose0的值
  changeChoose0: function (e) {
    ...
    this.inite2(index, this.data.inputvalue, this.data.shopType)
  },
  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function (options) {
    ...
    // var ka = that.data.ka
    // if (ka == 0 && addressInfo != '请选择定位信息') {
    //   app.globalData.addressInfo = '请选择定位信息'
    //   ka++
    //   that.setData({ ka: ka })
    // }
    // app.globalData.addressInfo = '请选择定位信息'
    ...
    // if (app.globalData.TablesNo != -999 && app.globalData.TablesNo != '' && app.globalData.TablesNo != undefined) {
    if (scene == "undefined") {
    ...
    if (app.globalData.userInfo.openId == "") {
      app.getUserInfo(function (e) {
        that.setData({ openId: e.openId, addressInfo: addressInfo, TablesNo: app.globalData.TablesNo, shopType: shopType })
        that.inite()//获取首页显示数据
        that.inite1()//获取分类列表
        that.inite2(0, that.data.goodsName, that.data.shopType) //首页菜单列表
      })
    } else {
      that.setData({ addressInfo: addressInfo, TablesNo: app.globalData.TablesNo, shopType: shopType })
      that.inite()
      that.inite1()
      that.inite2(0, that.data.goodsName, that.data.shopType)
    }
  ...
  chooseInfo: function (e) {
    ...
    // if (that.data.food.openState == 0 || that.data.msg == '' || that.data.msg == '配送范围外' || that.data.msg == '配送范围内') {
    //   if (that.data.msg == '' && app.globalData.TablesNo == -999) {
    //     wx.showToast({
    //       title: '请先选择地址',
    //       icon: 'loading'
    //     })
    //   } else if (that.data.msg == '配送范围外' && app.globalData.TablesNo == -999) {
    //     wx.showToast({
    //       title: '配送范围外',
    //       icon: 'loading'
    //     })
    //   } else if (that.data.food.openState == 0) {
    //     wx.showToast({
    //       title: '商家休息中',
    //       icon: 'loading'
    //     })
  ...
  RchooseInfo: function (e) {
    ...
    if (GASDetailList.length > 0) {
      ...
      that.RaddNums(index)
      // good.carCount++
      // item5.push({ name: good.GoodsName, size: that.data.firstInfo + ' ' + that.data.secondInfo + ' ' + that.data.thirdInfo, price: good.PriceStr, nums: good.carCount, goodsid: good.Id, attrSpacStr: that.data.allgroupindex, SpecInfo: that.data.selectAttrInfo, newCartRecord: 1 })
      // that.setData({ goodslist: goodslist,item5:item5 })
    }
    console.log('我是顺序id', index)
    console.log('我是菜品id', Id)
    console.log('我是菜品名字', goodname)
  ...
  setChoose: function (e) {
    ...
    if (this.data.groupindex1 > 0) {
      ...
      // var FirstInfoCount = tempdata1.find(f => f.id == allgroupindex)
      // this.data.FirstInfoCount = FirstInfoCount.count
      // if (this.data.FirstInfoCount == 0) {
      //   wx.showToast({
      //     title: '库存不足',
      //   })
      //   return
    ...
    if (this.data.goodslist[this.data.chooseNums].good.GASDetailList.length > 0) {
      ...
      if (attrmodel != undefined) {
        ...
        this.data.singleprice = attrmodel.priceStr
        // this.data.singlecount = attrmodel.count
        // this.data.sumprice = attrmodel.priceStr * this.data.buyamount
        // this.data.sumprice = this.data.sumprice.toFixed(2)
  ...
  setStorageCar: function () {
    ...
    if (isInfo) {
      ...
      if (item5.length > 0) {
        for (var i = 0; i < item5.length; i++) {
          if (item5[i].name == this.data.goodname && item5[i].SpecInfo == this.data.selectAttrInfo) {
            item5.splice(i, 1)
          }
        }
      }
      ...
      item5.push({ name: this.data.goodname, size: firstInfo + ' ' + secondInfo + ' ' + thirdInfo, price: price, nums: this.data.goodslist[this.data.chooseNums].good.carCount, goodsid: this.data.goodsid, attrSpacStr: this.data.allgroupindex, SpecInfo: this.data.selectAttrInfo, newCartRecord: 1, chooseNums: this.data.chooseNums })
      this.setData({
        edtiorNums: 0,
        edtiorNums1: 0,
        item5: item5,
      ...
      if (item5.length > 0) {
        ...
        // var findNums = item5.find(f => f.name == goodslist[index].good.GoodsName)
  ...
  addNums: function (chooseNums) {
    ...
    if (typeofa == 1) {
      if (item5.length > 0) {
        for (var i = 0; i < item5.length; i++) {
          if (item5[i].name == good.GoodsName) {
            item5.splice(i, 1)
          }
        }
      }
      item5.push({ name: good.GoodsName, size: this.data.firstInfo + ' ' + this.data.secondInfo + ' ' + this.data.thirdInfo, price: good.PriceStr, nums: good.carCount, goodsid: good.Id, attrSpacStr: this.data.allgroupindex, SpecInfo: this.data.selectAttrInfo, newCartRecord: 1, chooseNums: chooseNums })
      if (item5.length > 0) {
        for (var i = 0; i < item5.length; i++) {
          if (item5[i].nums == 0) {
            item5.splice(i, 1)
  ...
  addShopcarSingleNums: function (e) {
    ...
    if (item5.length > 0) {
      for (var i = 0; i < item5.length; i++) {
        ...
        // item5[i].price = parseFloat(item5[i].price).toFixed(2)
      }
    }
  ...
  RaddShopcarSingleNums: function (e) {
    ...
    shopCartlength--
    item5[index].price = (Number(item5[index].price) - (Number(item5[index].price) / Number(item5[index].nums + 1))).toFixed(2)
    // item5[index].price = parseFloat(item5[index].price).toFixed(2)
    // var price = Number(item5[index].price)
    // price = price.toFixed(2)
    for (var i = 0; i < item5.length; i++) {
      if (typeof (item5[i].price == "string")) {
        item5[i].price = Number(item5[i].price)
      }
      // item5[index].price = Number(item5[index].nums) * Number(this.data.item5_1[index].price)
      allprice += item5[i].price
      // parseFloat(item5[i].price).toFixed(2)
  ...
  // update: function () {
  //   for (var i = 0; i < this.data.item5.length; i++) {
  //     this.data.item5[i].price = parseFloat(this.data.item5[i].price).toFixed(2)
  //   }
  //   if (this.data.item5.length > 0) {
  //     var item5 = JSON.stringify(this.data.item5)
  //     wx.navigateTo({
  ...
  onShow: function () {
    ...
    if (app.globalData.isclearItem5 == 1) {
      this.inite1()
      this.inite2(0, '', this.data.shopType)
  ...
  onPullDownRefresh: function () {
    ...
    var shopCartlength = this.data.shopCartlength
    var allprice = this.data.allprice
    this.setData({ choose0: 0, inputvalue: '', item5: [], allprice: 0, shopCartlength: 0 })
    this.inite()
    this.inite1()
    this.inite2(0, '', this.data.shopType)
    wx.showToast({
  ...
  searchGood: function (e) {
    ...
    this.inite2(0, this.data.goodsName, this.data.shopType)
  },
  /**
   * 用户点击右上角分享
   */
  ...
  // 首页菜单列表
  inite2: function (typeid, goodsName, shopType) {
    var that = this
    wx.request({
  ...
  //         console.log('1', res)
  //       }
  //       if (that.data.goodsAttrList.length == 0) {
  //         that.setData({
  //           nums: nums + 1
  //         })
  //       } else {
