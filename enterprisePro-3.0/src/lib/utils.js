...
const canvasTools = {
  ...
  async pathCanvas(qrCode, target) {
    let m = {}
    let _g = _get(target, 'goodVm', '')
    let _group = _get(target, 'vm_group', '')
    let _ms = _get(target.vm, 'goodInfo', '')
    if (_g) {
      m.title = _g.name
      m.img = _g.img_fmt
      m.disprice = _g.priceStr
      m.discount = _g.discount
      m.price = _g.originalPrice
    } else if (_group) {
      m.img = _group.img
      m.title = _group.name
      m.discount = _group.discount
      m.disprice = _group.EntGroups.GroupPriceStr
      m.price = _group.EntGroups.OriginalPriceStr
    } else {
      m.discount = 99
      m.img = _ms.img
      m.title = _ms.name
 ...
 */
const un = {
  async icon(currentCom,index) {
    let vm = {}
    let share = currentCom.find(f => f.type == 'share')
    let music = currentCom.find(f => f.type == 'bgaudio')
    ...
    vm.music = music ? true : false
    vm.bgmSrc = music ? music.src : ""
