	...
	inite3: function () {
		...
		wx.request({
			...
			success: function (res) {
				if (res.data.obj.ADImg.length != 0) {
					var Imageurl = res.data.obj.ADImg[0].url 
				}
				app.globalData.shareObj = res.data.obj
				if (res.data.obj.ADImg.length == 0) {
					imgCanvas = 'http://img02.tooopen.com/images/20150928/tooopen_sy_143912755726.jpg'
				} else {
          imgCanvas = Imageurl.replace(/http/, "https")
				}
				...
				if (freeStyle == 0) {
                  ...
                  context.fillText(bottomText, windowWidth * 0.45, windowHeight * 0.98)
									context.draw()
				...
				if (freeStyle == 5) {
                  ...
                  var bottomText = "小未程序"
									...
									context.setFontSize(8)
									context.setFillStyle('#E8D9D9')
                  context.fillText(bottomText, windowWidth * 0.45, windowHeight * 0.97)
									context.draw()
								}
							})
						}
