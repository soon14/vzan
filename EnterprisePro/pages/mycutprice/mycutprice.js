...
Page({
  ...
  navtoOrderdetail: function (e) {
    ...
    tools.goNewPage('../bargainorderDetail/bargainorderDetail?orderId=' + index)
  },
  /**
   * 生命周期函数--监听页面加载
   */
  ...
  inite2: function (buid) {
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.isok) {
          tools.goNewPage('../orderListCutprice/orderListCutprice?BName=' + res.data.obj.BName + '&Freight=' + res.data.obj.Freight + '&ImgUrl=' + res.data.obj.ImgUrl + '&curPrcie=' + res.data.obj.curPrcie + '&buid=' + buid + '&goodId=' + that.data.goodId)
          // wx.navigateTo({
