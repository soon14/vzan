...
Page({
   ...
   */
	onLoad: function (options) {
		...
		page.memberInfo_2(function (res) {
			...
			.then(function (data) {
				if (data.isok == 1) {
					if (that.data.gobuy == true) {
						...
						parameters.push("isgroup=" + 0);
						parameters.push("groupid=" + that.data.GID);
						...
						tools.goNewPage('../orderList/group2_orderList?' + parameters.join("&") + '&GId=' + that.data.GID + '&joinGroup=' + that.data.joinGroup)
					}
				} else {
					wx.showToast({
						title: data.msg,
