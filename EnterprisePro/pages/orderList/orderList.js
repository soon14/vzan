...
Page({
	...
	data: {
		...
		couponsShow: false,
		money_coupon: 0,
		//length: wx.getStorageSync('addressLength'),
	},
	/**
	 * 生命周期函数--监听页面加载
	 */
	onLoad: function (options) {
		// console.log("打印出收货地址的长度",app.globalData.addressLength)

		// console.log(that.data.length)
		// that.data.length = wx.getStorageSync('addressLength')
		// console.log("打印出收货地址的长度", that.data.length)
		// var value = wx.getStorageSync('defaultAddress')
		// console.log("打印出相应地址",value.contact)
		// //如果用户设置了默认地址
		// if(that.data.length > 0){
		//   app.globalData.shippingAddress = value;
		//   console.log("显示地址", app.globalData.shippingAddress)
		...
		if (options.datajson != undefined) {
			...
			if (currentPage.length == 2 && currentPage[0].route == 'pages/detail/detail' || currentPage.length == 3 && currentPage[1].route == 'pages/detail/detail' || currentPage.length == 4 && currentPage[2].route == 'pages/detail/detail' || currentPage.length == 5 && currentPage[3].route == 'pages/detail/detail') {
				var menu = JSON.parse(currentPage[0].data.datajson || currentPage[1].data.datajson || currentPage[2].data.datajson || currentPage[3].data.datajson || currentPage[4].data.datajson || options.datajson)
			} else {
				menu = JSON.parse(currentPage[0].data.datajson || currentPage[1].data.datajson || currentPage[2].data.datajson || currentPage[3].data.datajson || currentPage[4].data.datajson || options.datajson)
			}
		...
		// console.log("设置收货地址")
		// that.data.Address.userName = app.globalData.defaultAddress.contact
		// that.data.Address.telNumber = app.globalData.defaultAddress.phone
		// console.log(app.globalData.defaultAddress.phone )
		// that.setData({
		//   Address: that.data.Address
		// })
		// console.log("Address的值", that.data.Address)

		//  that.freightRequest(that.data.goodCarIdStr)
		// that.AddressRequest()
	...
	onShow: function () {
		...
		// that.data.Address.userName = app.globalData.defaultAddress.contact
		// that.data.Address.telNumber = app.globalData.defaultAddress.phone
		// that.data.Address.provinceName = app.globalData.defaultAddress.province
		// that.data.Address.cityName = app.globalData.defaultAddress.city
		// that.data.Address.countryName = app.globalData.defaultAddress.district
		// that.data.Address.detailInfo = app.globalData.defaultAddress.street

		// console.log(app.globalData.defaultAddress.phone)
		// // that.setData({
		// //   Address: app.globalData.defaultAddress.contact
		// // })
		// console.log("Address的值", that.data.Address)
	...
	payFunc: function (e) {
		...
		if (that.data.Address.storeaddress != undefined && that.data.getWay == 0 || that.data.Address.detailInfo != undefined && that.data.getWay == 1) {
			console.log("Address的相应信息", that.data.Address)
			if (that.data.paymode == 0) {
				that.payRequest(1)    //微信支付
			} else {
				wx.showModal({
					title: '确认使用储值支付吗？',
					showCancel: true,
	...
	payRequest: function (buyMode) {
      ...
      console.log("用户到店自取的相关信息", Address.storeaddress)
		} else {
			var addressInfo = {
		...
		let salesManRecordId = getCurrentPages()[0].data.salesManRecordId || 0
		http
			.postAsync(addr.Address.addMiniappGoodsOrder, {
				...
				aid: wx.getStorageSync('PageSetting').msg.aid,
				//#region 新版拼团下单传递参数
				isgroup: options.isgroup || 0,
			...
			.then(function (data) {
				if (data.isok == 1) {
					...
					if (('money') in data && data.money === 0) {
						...
						setTimeout(function () {
							if (options.isgroup == 1) { //新版拼团跳转
								tools.goNewPage('../group2/orderSuccess?gOrderid=' + app.globalData.dbOrder)
							} else { //普通订单流程
						...
						setTimeout(function () {
							if (data.reductionCart == null) {
								...
								if (that.data.pickCoupon == null) {
									// var paymoney = Number(that.data.totalPrice) + Number(that.data.fee)
									// that.data.sumMoney = paymoney
									// that.setData({
									// 	sumMoney: (paymoney).toFixed(2)
									// })
								...
								if (options.isgroup == 1) { //新版拼团跳转
									...
									console.log("data.dbOrder", data.dbOrder)
