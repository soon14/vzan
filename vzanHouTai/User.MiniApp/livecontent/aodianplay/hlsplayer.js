...
function hlsplayerRun(conf){
	...
	//var playerwrapId = conf.playerwrap;
    ...
    THISHLS_SWF_NAME = "//j.vzan.cc/zhibo/livecontent/aodianplay/player.swf"; 
    xiSwfUrlStr = "//j.vzan.cc/zhibo/livecontent/aodianplay/playerProductInstall.swf";
		 ...
		 switch(info)
                     {
                       ...
                       case "hlsslider.end":
                            ...
                            break;
		               case "Play.Stop":
		                   if ($("#liveVideoAdUrl") && $("#liveVideoAdUrl").data("adurl") != undefined && $("#liveVideoAdUrl").data("adurl") != "" && ($("#liveVideoAdUrl").data("isvoer") == 0 || $("#liveVideoAdUrl").data("livestatus") == "-1")) {
		                       $("#liveVideoAdUrl").data("isvoer", 1);
		                       setTimeout(function () {
		                           if ($("#liveVideoAdUrl").data("livestatus") != "-1") {
		                               _this.handle.changePlayer($("#liveVideoAdUrl").data("liveVideoUrl"));
		                               try { _this.handle.startPlay(); } catch (ex) { }
		                           }
		                           else {
		                               $("#live-status").html("未开始");
		                           }
		                           $("#liveVideoAdUrl").remove();
		                       }, 500);
	...
	this.onSwfReady = function () {	
		...
		var swfobj = swfobject.getObjectById(this.uuid);
		var _default_video_size = {
		    width: 0,
		    height: 0,
		}
		var id = window.setInterval(function () {
		    //self.setPlayStreamInfo()
		    //_default_video_size.height = swfobj.getVideoHeight()
		    var _h = swfobj.getVideoHeight(),
              _w = swfobj.getVideoWidth();
		    //var _docwidth = document.querySelector("#" + playerwrapId).offsetWidth;

		    if (_h != 0 && _h != _default_video_size.height ) {
		        //console.log(_w, _h, _docwidth);
		        //_default_video_size.width = _w;
		        //_default_video_size.height = _h;
		        //var _cal_h = _h * (_docwidth / _w);

		        //if (window.zbvd && !window.zbvd.ispc) {
		        //    if (window.zbvd.tplname != 'Vertical') {
		        //        document.querySelector("#hlsMedia1").style.height = _cal_h + "px";
		        //    }
		        //}
		        //else {
		        //    try {
		        ...
		        //if (window.zbvd&&!zbvd.ispc) {
		        //    zbvd.isvideochange = true;
		        //}
		        //document.querySelector("#hlsMedia1").style.height = _cal_h + "px";//这段代码自动调整视频尺寸
