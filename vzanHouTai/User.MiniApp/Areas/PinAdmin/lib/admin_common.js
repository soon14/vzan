; $(function () {
	...
	*/
    ...
    $('body').delegate(".upload-pre-item22 em", "click", function () {
        $(this).parent().remove();

        var id = $(this).data("id");
        if (id != "" && id != undefined) {
            $.ajax({
                url: "/stores/DeleteImg",
                data: { 'id': id },
                dataType: 'Json',
                success: function (data) {
                    console.log("删除图片成功");
...
function uploadImgDialog(obj, opts) {
    ...
    if (maxCount > 1) {
        maxCount = maxCount - $(obj).siblings(".upload-img-view").find(".upload-pre-item22").length;
    }
    var field = $(obj).attr('rel');
    var width = $(window).width() * 0.8, height = $(window).height() * 0.8;
    ...
    uploadObj = obj;
    $uploadHtml = '<div>' +
        '<div class="upload_dialog" style="height:' + iframeHeight + 'px;overflow-y:hidden;overflow-x:hidden;"><div>' +
        '<iframe id="goodsIframe" name="goodsIframe" style="height:' + iframeHeight + 'px;width:100%; border:none" border="0" src="' + UPLOAD_URL + '&objKey=' + field + '&remainCount=' + maxCount + '"></iframe>' +
        '</div>';
    $.Dialog.open("上传图片", { width: width, height: height }, $uploadHtml);
}
