//dom加载完成后执行的js
 $(function () {
    ...
    //限制input只能输入2位小数的folat
    $(".folat-input").keyup(function () {
	...
	// 分页跳转页面
	$(".targetPageSearch").click(function(){
		var totlePage =  $(this).attr('totlePage');
		var totleNum = (/p\/(\d+)\.html/).exec(totlePage)[1] || 1;
		var targetNum = $('#pageInput').val() || totleNum;
		totleNum = parseInt( totleNum );
		if ( targetNum <= 0 || targetNum > totleNum ) {
			alert('页码超出范围，请重新填写！');
			return;
		} else {
			location.href = totlePage.replace(/p\/(\d+)\.html/, 'p/' + targetNum + '.html');
		}
	});
    ...
    $('.ajax-get').click(function(){
        ...
        if ( $(this).hasClass('confirm') ) {
        	var confirm_text=$(this).attr('c_text');
        	var text='确认要执行该操作吗?';
        	if(confirm_text){
        		text=confirm_text;
        	}
	...
	//ajax post submit请求
    $('.ajax-post-close').click(function(){
        var target,query,form;
        var target_form = $(this).attr('target-form');
        var that = this;
        var nead_confirm=false;
                    ...
                    if($(that).hasClass('dialog_submit')){
                        ...
                        setTimeout(function(){
                            if (data.url) {
                                ...
                                window.parent.document.getElementById("closeEditLayer").click();
                            }
                        },1500);
                    }
                }else{
...
function onUploadFileSuccess(file, data, name){
	data = $.parseJSON(data);
	if(data.status){
		var input = $("input[name=" + name + "]"), parent = input.parent(), type = parent.data('type') || "id";
		if ("path" === type) {
			input.val(data.url ? data.url : data.path);
		} else {
			input.val(data.id);
		}
		parent.find('.upload-img-box').html("<div class=\"upload-pre-file\"><span class=\"upload_icon_all\"></span>" + data.name + "</div>");
	} else {
		//console.log(data);
		updateAlert(data.info);
		setTimeout(function(){
...
(function(){
	...
	*/
	function openSelectUsers(dataUrl,count,callback){
		...
		var $contentHtml = $('<div><div class="goods_dialog" style="padding:10px; height:530px;overflow-y:hidden;overflow-x:hidden;">' +
			'<div class="mt_10">' +
			'<iframe id="usersIframe" name="usersIframe" style="height:530px;width:100%; border:none" border="0" src="' + dataUrl + '"></iframe>' +
			'</div></div>' +
			'<div class="btn_bar">' +
			'<a href="javascript:;" class="btn btn-primary btn-confirm">确定</a>&nbsp;&nbsp;' +
			'<a href="javascript:;" class="btn btn-info btn-cancel">取消</a>' +
			'</div></div>');
		$.Dialog.open("选择用户", {width: 1000, height: 640}, $contentHtml);
		
		$('.btn-cancel',$contentHtml).click(function(){
			$.Dialog.close();
		});
		$('.btn-confirm',$contentHtml).click(function(){
			var trs = $(window.frames["usersIframe"].document, $contentHtml).find("table tr");
			var usresList = [];
			trs.each(function (index, element) {
				if ($(element).find(".ids").prop("checked")) {
	...
	function selectSingleUser(dataUrl,name){
		$.WeiPHP.openSelectUsers(dataUrl,1,function(data){
			if(data && data.length>0){
				for(var i=0;i<data.length;i++){
					var $html = $('<div class="photo-item" onClick="$.WeiPHP.selectSingleUser(\''+dataUrl+'\',\''+name+'\')">'+
						'<img src="'+data[i].img+'"/>'+
						'<input type="hidden" name="'+name+'" value="'+data[i].id+'"/>'+
						'<span class="remove fa fa-times" onclick="$(this).parent().remove();event.stopPropagation();"></span>' +
						'<span class="photo-title">'+data[i].nickname+'</span>'+
						'</div>');
					$('#userList').html($html);
				}
			}
			...
			if (data && data.length > 0) {
				...
				for (var i = 0; i < data.length; i++) {
					var $html = $('<div class="photo-item" onclick="$.WeiPHP.selectSingleUser(' + dataUrl + ',' + name + ')">' +
						...
						'<span class="photo-title">' + data[i].nickname + '</span>' +
						'</div>');
					$('#userList .photo-item-add').before($html);
				}
			}
		});
	}
...
//看是否为folat类型,此处限制2位小数
function CheckFolat_D2(e) {
    var obj = $(e);
    if (!/^[0-9]{1,10}(\.\d{0,2})?$/.test(obj.val())) {
        layer.msg("请输入正确的数值", { time: 1000 });
        obj.val("");
        return;
    }
}