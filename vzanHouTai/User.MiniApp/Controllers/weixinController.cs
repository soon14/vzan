...
namespace User.MiniApp.Controllers
{
    ...
    public class weixinController : Controller
    {
        ...
        public static readonly string EncodingAESKey = Senparc.Weixin.Config.SenparcWeixinSetting.EncodingAESKey;//与微信公众账号后台的EncodingAESKey设置保持一致，区分大小写。
        public static readonly string AppId = Senparc.Weixin.Config.SenparcWeixinSetting.WeixinAppId;//与微信公众账号后台的AppId设置保持一致，区分大小写。
        // GET: weixin
        [HttpGet]
        [ActionName("Index")]
        ...
        public ActionResult CreateQrCode(string code)
        {
            var qrResult = Senparc.Weixin.MP.AdvancedAPIs.QrCodeApi.Create(AppId, 300, 12, QrCode_ActionName.QR_STR_SCENE, code);
           // var qrCodeUrl = Senparc.Weixin.MP.AdvancedAPIs.QrCodeApi.GetShowQrCodeUrl(qrResult.ticket);
            if (!string.IsNullOrEmpty(qrResult.url))
            {
                string Logo = "http://j.vzan.cc/dz/pxhcontent/image/pinxianghui/white-logo.png";
                MemoryStream ms = new MemoryStream();
                Bitmap bmp = QRCodeHelp.Instance.GetQrCodeImg(Logo, qrResult.url);
                bmp.Save(ms, ImageFormat.Jpeg);
                bmp.Dispose();
                return File(ms.ToArray(), "image/jpg");
            }
            return Json(qrResult);
        }
    }
}