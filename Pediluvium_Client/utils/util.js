...
function AddOrder(param, refun, reparam) {
	...
	wxPayRequst(param, {
		failed: function (res) {
			...
			console.log("生成订单失败")
			wx.hideNavigationBarLoading()
			refun(res, JSON.stringify(param))
		},
		success: function (res) {
			console.log(res)
			if (res == "wxpay") {
...
function wxPayRequst(param, callback) {
	var that = this
	var url = addr.Address.AddPayOrder_Store
	wx.request({
		url: url,
		data: param,
		method: 'POST',
		...
		success: function (res) {
			if (res.data.isok > 0) {
				if (res.data.msg == '订单生成并支付成功') {
					template.UpdateWxCard(that)
					template.goNewPage('../paysuccess/paysuccess?payway=' + param.payWay)
					return
				}
				getApp().globalData.orderid = res.data.dbOrder
...
function PayOrder(orderid, param, pay_callback) {
	console.log('payorder参数', param.openId, orderid, 1)
	var that = this
	wx.request({
		...
		success: function (res) {
			...
			if (res.data.result == true) {
				...
				if (res.data.msg == '下单异常') {
					template.showmodal('提示', res.data.obj, false, 1)
					return
				} else {
...
function wxpay(param, callback) {
	...
	wx.requestPayment({
		...
		complete: function (res) {
			// callback.
			// if (res.errMsg == 'requestPayment:cancel' && ("failed" in callback)) { //支付取消
			// }
		}
	})
}
