  ...
  wx.request({
    ...
    success: function (res) {
      ...
      if (res.data.isok == 1) {
        ...
        for (var i = 0; i < comment.length; i++) {
          ...
          if (item.SubCommentList.length > 0) {
            for (var j = 0; j < item.SubCommentList.length; j++) {
              var sitem = item.SubCommentList[j]
              var sdata = htmljson.html2json(sitem.ContentHtml, 'commentContent' + sitem.Id);
        ...
        content.comment = content.comment.concat(comment)
        content.commentdataname = commentdataname
        content.subcommentdataname = subcommentdataname
        content.qqfacehidden = true
        if (that.data.typepl == undefined || that.data.typepl == '' || that.data.typepl =='undefined')
        {
  ...
  wx.request({
    ...
    success: function (res) {
      ...
      if (res.data.isok) {
        ...
        //获取子评论列表
        var sitemname = []
        if (res.data.SubCommentList.length > 0) {
          for (var j = 0; j < res.data.SubCommentList.length; j++) {
            var sitem = res.data.SubCommentList[j]
            var sdata = htmljson.html2json(sitem.ContentHtml, 'commentContent' + sitem.Id);
            sitemname.push(sdata)
          }
        }
        content.subcommentdataname[index] = sitemname
        content.comment[index].SubCommentList = res.data.SubCommentList
        that.data.items[0].content = content
...
function getqqfacedata(that) {
  for (var i = 0; i < 100; i++) {
    ...
    that.data.items[0].content.qqfacedata.push(item)
  }
  that.setData({
    items: that.data.items
  })
...
function commentDidClick(that, params) {
  ...
  content.icon = '../../image/tc-yh-07.png'
