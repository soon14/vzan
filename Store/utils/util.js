	...
	wx.request({
		...
		success: function (res) {
			if (res.data.isok > 0) {
				...
				getApp().globalData.orderTwiceId = oradid
				getApp().globalData.newparam = newparam
				if (getApp().globalData.payType == 2) { //余额支付
					wx.showToast({
						title: '支付成功',
					})
					wx.redirectTo({
						url: '../paysuccess/paysuccess?totalMoney=' + getApp().globalData.sumprice + '&buyMode=2',
					})
					UpdateWxCard(this)
					return
				}
			...
			//         PayOrder(getApp().globalData.orderTwiceId, getApp().globalData.newparam, {
			//           failed: function () {
			//             callback.failed("failed")
			//           },
			//           success: function (res) {
