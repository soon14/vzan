...
Page({
  ...
  initgetGoodsInfoData:function(data){
    var that = this;
    if(data.code == 1){
        that.setData({goods_data:data.info});
        WxParse.wxParse('g_description', 'html', data.info.g_description, that,5);
  ...
  initgetCartListData:function(data){
    ...
    bind_cart_number_jian: function (e) {
        ...
        requestData.table_id = wx.getStorageSync('global_table_id' + that.data.this_dish_id);
