...
var core = {
	...
	//查询技师信息
	GetTechInfo: function (parameters) {
		return new Promise(function (resolve, reject) {
			var techInfo = wx.getStorageSync("techInfo");
			if (techInfo) {
				resolve(techInfo);
			}
			else {
				http.get(addr.Address.GetTechInfo, parameters).then(function (res) {
					if (res && res.isok) {
						techInfo = {
							userid: res.dataObj.userInfo.id,
							nickname: res.dataObj.userInfo.jobNumber,
							headimg: res.dataObj.userInfo.headImg
						};
						wx.setStorageSync("techInfo", techInfo)
						resolve(techInfo);
					}
					else {
						wx.showModal({
