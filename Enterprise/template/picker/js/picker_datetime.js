...
var pickerDatetime = function (obj) {
  ...
  var _this = this;
  this.page = obj.page;
  app.globalData.isFirstPickTime = 0
...
pickerDatetime.prototype.setPicker = function (pickerName) {
  ...
  console.log("beginTime", this.page.data["beginTime"]);
  if (this.page.data["beginTime"]) {
    var t = this.page.data["beginTime"].replace('年', '/').replace('月', '/').replace('日', '');
    console.log(t);
    var pickerPreDate = new Date(t);
    ...
    var obj = new Object();
    obj[pickerName] = pickerPreDate.Format("yyyy年MM月dd日 HH:mm:ss"); //pickerPreYear + '年' + pickerPreMonth + '月' + pickerPreDay + '日 ' + addDatetimeZero(pickerPreHour) + ':' + addDatetimeZero(pickerPreMinute)+":00";
  ...
  // if (app.globalData.isFirstPickTime == 0) {
  //   newObj['pickDatetimeValue'] = [pickerDateIndex, pickerHourIndex, pickerMinuteIndex];
  //   this.pickDatetimeValue_1 = [pickerDateIndex, pickerHourIndex, pickerMinuteIndex];
  //   ++app.globalData.isFirstPickTime
  // } else {
  //   newObj['pickDatetimeValue'] = this.pickDatetimeValue_1
  // }
  newObj['pickDatetimeValue'] = [pickerDateIndex, pickerHourIndex, pickerMinuteIndex];
  this.pickDatetimeValue_1 = [pickerDateIndex, pickerHourIndex, pickerMinuteIndex];

  console.log(newObj['pickDatetimeValue']);
  ...
  this.page.bindChange = function (e) {
    ...
    var picktime = newObj[pickerName].replace('年', '/').replace('月', '/').replace('日', '');
    //
    if (_this.page.data['beginTime']) {
      var starttime = _this.page.data['beginTime'].replace('年', '/').replace('月', '/').replace('日', '');
      picktime = new Date(picktime);
      starttime = new Date(starttime);
      var timespan = picktime - starttime;
      var timespanMinute = timespan / (1000 * 60);
      console.log(timespanMinute);
      if (timespanMinute < 0) {
        wx.showModal({
          title: '提示',
          content: '选择时间不能早于' + _this.page.data['beginTime'],
        })
        _this.setPicker(pickerName);
        return;
      }
    ...
    //
    this.setData(newObj);
    //console.log(picktime, starttime);
